...
def build_trainer(opt, device_id, model, fields,
    ...
    trainer = onmt.Trainer(model, train_loss, valid_loss, optim, trunc_size,
                           ...
                           model_saver=model_saver, use_refer=opt.refer,
                           use_mask=opt.mask_attention,
...
class Trainer(object):
    ...
    def __init__(self, model, train_loss, valid_loss, optim,
                 ...
                 gpu_verbose_level=0, report_manager=None, model_saver=None, use_refer=False,
                 use_mask=False, mask_path=None):
        # Basic attributes.
        self.model = model
        ...
        self.refer = use_refer
        self.use_mask = use_mask

        if self.use_mask:
            train_mask_file, valid_mask_file = mask_path.split(':')
            with open(train_mask_file) as fpt, open(valid_mask_file) as fpv:
                self.train_masks = [eval(mask.strip()) for mask in fpt.readlines()]
                self.valid_masks = [eval(mask.strip()) for mask in fpv.readlines()]
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            if self.use_mask:
                length = src.size(0)
                indices = batch.indices.tolist()
                masks = [self.train_masks[i][:length] + [0] * (length - len(self.train_masks[i])) for i in indices]
                masks = torch.tensor(masks, device=src.device).t().unsqueeze(-1)
            else:
                masks = None
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if self.grad_accum_count == 1:
                    self.model.zero_grad()
                outputs, attns = \
                    self.model(src, tgt, src_lengths, ref, masks=masks)
