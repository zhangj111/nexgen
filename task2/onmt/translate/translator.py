...
class Translator(object):
    ...
    def __init__(self,
        ...
        self.use_mask = opt.mask_attention
        if self.use_mask:
            translate_mask_file = opt.mask_path
            with open(translate_mask_file) as fpt:
                self.translate_masks = [eval(mask.strip()) for mask in fpt.readlines()]
    ...
    def _run_refer(self, batch, data_type):
        ref = inputters.make_features(batch, 'ref', data_type)
        _, ref_lengths = eval('batch.ref')

        ref_lengths, ind = ref_lengths.sort(descending=True)
        sorted_ref = ref[:, ind, :]
        enc_states, memory_bank, ref_lengths = self.model.refer(
        ...
        if ref_lengths is None:
            ...
            ref_lengths = torch.Tensor(batch.batch_size) \
                               ...
                               .long() \
                               .fill_(memory_bank.size(0))
        return ref, enc_states, memory_bank, ref_lengths

    def get_masks(self, batch, src):
        length = src.size(0)
        indices = batch.indices.tolist()
        masks = [self.translate_masks[i][:length] + [0] * (length - len(self.translate_masks[i])) for i in indices]
    ...
    def _decode_and_generate(self, decoder_input, memory_bank, batch, data,
                             ...
                             step=None, batch_offset=None, ref_bank=None, ref_lengths=None, masks=None):
    ...
    def _fast_translate_batch(self,
        ...
        if self.refer:
            ref, ref_states, ref_bank, ref_lengths = self._run_refer(batch, data.data_type)
        else:
            ref, ref_states, ref_bank, ref_lengths = tuple([None]*4)
        ...
        if self.refer:
            if isinstance(ref_bank, tuple):
                ref_bank = tuple(tile(x, beam_size, dim=1) for x in ref_bank)

            else:
                ref_bank = tile(ref_bank, beam_size, dim=1)

            ref_lengths = tile(ref_lengths, beam_size)

        if self.use_mask:
            masks = self.get_masks(batch, src)
            masks = tile(masks, beam_size, dim=1)
        else:
            masks=None

        top_beam_finished = torch.zeros([batch_size], dtype=torch.uint8).bool()
        ...
        for step in range(max_length):
            ...
            log_probs, attn = \
                self._decode_and_generate(decoder_input, memory_bank,
                                          ...
                                          src_map=src_map,
                                          step=step,
                                          batch_offset=batch_offset, ref_bank=ref_bank,
            ...
            if self.refer:
                if isinstance(ref_bank, tuple):
                    ref_bank = tuple(x.index_select(1, select_indices)
                                        for x in ref_bank)
                else:
                    ref_bank = ref_bank.index_select(1, select_indices)

                ref_lengths = ref_lengths.index_select(0, select_indices)

            if self.use_mask:
                masks = masks.index_select(1, select_indices)
