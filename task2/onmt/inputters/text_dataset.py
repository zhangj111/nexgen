...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
                 ...
                 dynamic_dict=False, use_filter_pred=True, ref_examples_iter=None, num_ref_feats=0, ignore_unk=False):
        self.data_type = 'text'
        ...
        self.n_ref_feats = num_ref_feats
        self.n_tgt_feats = num_tgt_feats

        # Each element of an example is a dictionary whose keys represents
        # at minimum the src tokens and their indices and potentially also
        # the src and tgt features and alignment information.
        if ref_examples_iter is not None:
            if tgt_examples_iter is not None:
                examples_iter = (self._join_dicts(src, ref, tgt) for src, ref, tgt in
                                 zip(src_examples_iter, ref_examples_iter, tgt_examples_iter))
            else:
                examples_iter = (self._join_dicts(list(y for x in item for y in (x if isinstance(x, tuple) else (x,)))) for item in
                                 zip(src_examples_iter, ref_examples_iter))
        ...
        out_fields = [(k, fields[k]) if k in fields else (k, fields['ref'])
                      for k in keys]
        ...
        for ex_values in example_values:
            ...
            if ignore_unk:
                example.src = tuple([word for word in example.src if word in out_fields[0][1].vocab.stoi])
    ...
    @staticmethod
    def get_fields(n_src_features, n_tgt_features, lower=False):
        """
        Args:
        ...
        fields["src"] = torchtext.data.Field(
            pad_token=PAD_WORD, lower=lower,
            include_lengths=True)

        fields["ref"] = torchtext.data.Field(
            pad_token=PAD_WORD, lower=lower,
            include_lengths=True)
