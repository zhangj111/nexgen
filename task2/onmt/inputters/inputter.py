...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
                  ...
                  image_channel_size=3, ref_data_iter=None, ref_path=None, ref_seq_length_trunc=0, ignore_unk=False):
    """
    ...
    if ref_path or ref_data_iter:

        ref_examples_iter, num_ref_feats = \
                TextDataset.make_text_examples_nfeats_tpl(
                    ref_data_iter, ref_path, ref_seq_length_trunc, "ref")
    else:
        ref_examples_iter, num_ref_feats = None, None
    ...
    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter,
                              ...
                              ref_examples_iter=ref_examples_iter, num_ref_feats=None, ignore_unk=ignore_unk)
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                ...
                ref_vocab_path=None, ref_vocab_size=0, ref_words_min_frequency=0):
    """
    Args:
    ...
    ref_vocab = load_vocabulary(ref_vocab_path, tag="reference")
    if ref_vocab is not None:
        ref_vocab_size = len(ref_vocab)
        logger.info('Loaded reference vocab has %d tokens.' % ref_vocab_size)
        for i, token in enumerate(ref_vocab):
            counter['ref'][token] = ref_vocab_size - i
    ...
    if data_type == 'text':
        if ref_vocab_size:
            _build_field_vocab(fields["ref"], counter["ref"],
                               max_size=ref_vocab_size,
                               min_freq=ref_words_min_frequency)
            logger.info(" * ref vocab size: %d." % len(fields["ref"].vocab))
        ...
        if share_vocab:
            ...
            # merged_vocab.extend(fields["tgt"].vocab)
